...
import Data.Coders (decodeList, decodeRecordSum, decodeSet, encodeFoldable, invalidKey)
...
-- | Validity interval.
--
--   TODO this should be defined along with `TxBody` once that's defined.
...
instance NoThunks ValidityInterval

instance FromCBOR ValidityInterval

instance ToCBOR ValidityInterval

-- | Does the slot lie within the validity interval?
inInterval :: SlotNo -> ValidityInterval -> Bool
inInterval _ (ValidityInterval SNothing SNothing) = True
inInterval slot (ValidityInterval SNothing (SJust to)) = slot <= to
inInterval slot (ValidityInterval (SJust from) SNothing) = slot >= from
inInterval slot (ValidityInterval (SJust from) (SJust to)) =
  from <= slot && slot <= to
...
instance
  ...
  STS (UTXO (ShelleyMAEra (ma :: MaryOrAllegra) c))
  where
